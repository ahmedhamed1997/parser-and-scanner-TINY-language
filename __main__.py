...
class TINYParserWidget(QWidget):
    ...
    def initUI(self):
        ...
        submit_button2 = QPushButton('scan')
        submit_button2.clicked.connect(self.scan)
        submit_button.clicked.connect(self.submitted)
        grid = QGridLayout()
        grid.setSpacing(10)
        grid.addWidget(lbl, 1, 0)
        self.output_code = QTextEdit()
        grid.addWidget(self.output_code, 2, 1)
        grid.addWidget(self.input_code, 1, 1)
        grid.addWidget(submit_button, 4, 1)
        grid.addWidget(submit_button2, 3, 1)
        self.setLayout(grid)
        self.setGeometry(40, 40, 500, 900)
        self.setWindowTitle('TINY Parser and Scanner')
        self.show()
    ...
    def outcode(self):
        ...
        if outf.mode == 'r':
            contents = outf.read()
            self.output_code.setPlainText(contents)


    def setScan(self):
        text = self.input_code.toPlainText()
        f = open('TINY_in_code.txt', 'w')
        f.write(text)
    ...
    def draw(self, same_rank_nodes):
        ...
        for shape in ['s', 'o']:
            nx.draw_networkx_nodes(graph, pos, node_color='gainsboro', node_size=1300, node_shape=shape, labels=labels, nodelist=[
